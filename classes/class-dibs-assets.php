	...
	public function dibs_load_js() {
		...
		if ( is_checkout() && ! is_wc_endpoint_url( 'order-pay' ) ) {
			...
			if ( WC()->session->get( 'dibs_payment_id' ) ) {
				...
				if ( empty( $easy_confirm ) ) { //phpcs:ignore
			...
			$src = WC_DIBS__URL . '/assets/js/dibs-easy-for-woocommerce' . $script_version . '.js';
			wp_register_script(
				'checkout',
				$src,
				array(
			...
			wp_localize_script(
				...
				array(
					...
					'log_to_file_url'                  => WC_AJAX::get_endpoint( 'dibs_wc_log_js' ),
					'log_to_file_nonce'                => wp_create_nonce( 'dibs_wc_log_js' ),
