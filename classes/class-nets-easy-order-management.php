...
class Nets_Easy_Order_Management {
	...
	public function dibs_order_completed( $order_id ) {
		...
		if ( in_array( $gateway_used, nets_easy_all_payment_method_ids(), true ) ) {
			...
			if ( 'A2A' === $payment_type ) {
				...
				/* Translators: Nexi payment method for the order. */
			...
			$wc_order->add_order_note( sprintf( __( 'Payment charged in Nexi Checkout with charge ID %s', 'dibs-easy-for-woocommerce' ), $response['chargeId'] ) ); // phpcs:ignore
	 ...
	 *
	 * @param array $nets_easy_order The Nexi easy order.
	...
	public function is_canceled( $nets_easy_order, $order_id ) {
		...
		if ( ! is_wp_error( $nets_easy_order ) && isset( $nets_easy_order['payment']['summary'] ) ) {
			...
			if ( $canceled_amount ) {
				...
				$order->add_order_note( sprintf( __( 'Payment canceled in Nexi Checkout ( Portal ) with Payment ID %1$s. Payment type - %2$s. Charge ID %3$s.', 'dibs-easy-for-woocommerce' ), $nets_easy_order['payment']['paymentId'], $nets_easy_order['payment']['paymentDetails']['paymentMethod'], $canceled_amount ) );
				$order->save();
				return true;
			}
		}
	...
	public function is_completed( $nets_easy_order, $order_id ) {
		...
		if ( ! is_wp_error( $nets_easy_order ) && ! empty( $nets_easy_order['payment']['charges'] ) ) {
			...
			if ( $dibs_charge_id ) {
				...
				$wc_order->save();
				// Translators: 1. Nexi Checkout Payment id 2. Payment type  3.Charge id.
				$wc_order->add_order_note( sprintf( __( 'Payment charged in Nexi Checkout ( Portal ) with Payment ID %1$s. Payment type - %2$s. Charge ID %3$s.', 'dibs-easy-for-woocommerce' ), $nets_easy_order['payment']['paymentId'], $nets_easy_order['payment']['paymentDetails']['paymentMethod'], $dibs_charge_id ) );
				return true;
			}
		}
