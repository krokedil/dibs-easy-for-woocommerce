...
class DIBS_Api_Callbacks {
    ...
    public function payment_created_scheduler() {
        ...
        error_log('schedule_payment_created_cb ' . var_export($data, true));
        
	}

	public function execute_dibs_payment_created_callback( $data, $order_id = '' ) {
		
		DIBS_Easy::log( 'Payment created callback. Order ID:' . $order_id ); // Input var okay.
		DIBS_Easy::log( 'Payment created. Response data:' . json_encode( $data ) );
		
		if( empty( $order_id ) ) { // We're missing Order ID in callback. Try to get it via query by internal reference
			$order_id = $this->get_order_id_from_internal_reference( $data['paymentId'] );
		}
	...
	public function get_order_id_from_internal_reference( $internal_reference ) {
	    ...
	    foreach( $orders as $order_id ) {
			...
			$order_internal_reference = get_post_meta( $order_id, '_dibs_payment_id', true );
			
	        if( $order_internal_reference === $internal_reference ) {
	            $order_id_match = $order_id;
	            DIBS_Easy::log('Order ID is missing in OSN callback but Internal reference ' . $internal_reference . '. already exist in order ID ' . $order_id_match);
	            break;
	        }
		}
